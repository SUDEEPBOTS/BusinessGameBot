...
async def properties_command(client, message: Message):
    ...
    for p in game.players:
        if p.user_id == message.from_user.id:
            ...
            break
    if not player:
        return await app.send_message(message.chat.id, "You are not playing in this game!")
    if not player.properties:
        return await app.send_message(message.chat.id, "You don't own any properties.")
...
async def sell_command(client, message: Message):
    ...
    if len(message.command) < 2:
        return await app.send_message(message.chat.id, "Please provide a property ID. Usage: `/sell <ID>`\nUse `/properties` to see IDs.")
    try:
        pos = int(message.command[1])
    except ValueError:
        return await app.send_message(message.chat.id, "Invalid property ID. Must be a number.")
    player = None
    for p in game.players:
        if p.user_id == message.from_user.id:
            player = p
    ...
    if pos not in player.properties:
        return await app.send_message(message.chat.id, "You don't own this property!")
    prop = BOARD_SPACES[pos]
    sell_value = prop['price'] // 2
    del player.properties[pos]
    player.balance += sell_value
    await app.send_message(message.chat.id, f"✅ **{player.name} sold {prop['name']} back to the bank for ${sell_value}!**\nNew Balance: ${player.balance}")
...
async def trade_command(client, message: Message):
    ...
    chat_id = message.chat.id
    if chat_id not in ACTIVE_GAMES:
        return await app.send_message(message.chat.id, "No active game.")
    game = ACTIVE_GAMES[chat_id]
    
    if not message.reply_to_message:
        return await app.send_message(message.chat.id, "Please reply to the player you want to trade with.\nUsage: `/trade <property_id> <price_to_sell>`")
        
    args = message.command
    if len(args) < 3:
        return await app.send_message(message.chat.id, "Usage: `/trade <property_id> <price_to_sell>`")
        
    try:
        prop_id = int(args[1])
        price = int(args[2])
    except ValueError:
        return await app.send_message(message.chat.id, "Property ID and Price must be numbers.")
        
    if price < 0:
        return await app.send_message(message.chat.id, "Price cannot be negative.")
        
    sender_id = message.from_user.id
    target_id = message.reply_to_message.from_user.id
    
    if sender_id == target_id:
        return await app.send_message(message.chat.id, "You can't trade with yourself!")
    ...
    if not sender or not target:
        return await app.send_message(message.chat.id, "Both users must be playing the game.")
